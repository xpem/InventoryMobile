...
namespace ApiDAL
{
    public class HttpClientFunctions : HttpClient
    {
        ...
        public static async Task<ApiResponse> Request(Models.RequestsTypes requestsType, string url, string? userToken = null, string? jsonContent = null)
        {
            try
            {
                using HttpClient httpClient = new();
                ...
                switch (requestsType)
                {
                    case Models.RequestsTypes.Get:
                        httpResponse = await httpClient.GetAsync(url);
                        break;
                    case Models.RequestsTypes.Post:
                    ...
                    case Models.RequestsTypes.Put:
                        ...
                        break;
                    case Models.RequestsTypes.Delete:
        ...
        public static async Task<ApiResponse> AuthRequest(Models.RequestsTypes requestsType, string url, string? jsonContent = null)
        {
            bool retry = true;
        ...
        private static async Task<(bool success, string? newToken)> RefreshToken()
        {
            ...
            if (user is not null && user.Email is not null && user.Password is not null)
            {
                ...
                {
                    UserLocalDAl.UpdateToken(user.Id, newToken);
